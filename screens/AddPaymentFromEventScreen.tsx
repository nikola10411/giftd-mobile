...
interface IAddPaymentFromEventScreenProps {
  navigation: NavigationProp<any>;
}

const AddPaymentFromEventScreen: React.FC<IAddPaymentFromEventScreenProps> = (
  props
) => {
  ...
  const goAddPaymentBankScreen = () => {
    navigation.navigate('AddPaymentBank');
  };
  ...
  useEffect(() => {
    if (isAddingPaymentMethodSuccess) {
      setValidationSuccessMessage(
        'The new payment method has been created successfully.'
      );
      setCardNumber('');
      setExpMonth('');
      setExpYear('');
      setCVC('');
      dispatch(clearPaymentMethodState());
      navigation.navigate('SelectPaymentMethodFromEventScreen');
    }
  }, [isAddingPaymentMethodSuccess]);
...
export default AddPaymentFromEventScreen;
