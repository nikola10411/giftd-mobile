...
const PaymentMethodScreen: React.FC<IPaymentMethodScreenProps> = (props) => {
    ...
    const goAddPaymentScreen = () => {
        // navigation.navigate('AddPaymentInSetting');
        navigation.navigate('PaymentMethodSelectionInSettingScreen');
    };
                    ...
                    <ScrollView>
                        {
                            paymentMethodList.length === 0 ? (
                                ...
                                <>
                                    ...
                                    {
                                        paymentMethodList.filter((paymentMethod) => paymentMethod.type == 'card').length === 0 && (
                                            <Text style={[styles.noteInfo, { fontFamily: 'Work-Sans-regular' }]} >
                                                No card added yet
                                            </Text>
                                        )
                                    }
                                    {
                                        paymentMethodList.filter((paymentMethod) => paymentMethod.type == 'card').map((paymentMethod, index) => {
                                            return (
                                    ...
                                    {
                                        paymentMethodList.filter((paymentMethod) => paymentMethod.type == 'bank').length === 0 && (
                                            <Text style={[styles.noteInfo, { fontFamily: 'Work-Sans-regular' }]} >
                                                No bank added yet
                                            </Text>
                                        )
                                    ...
                                    {
                                        paymentMethodList.filter((paymentMethod) => paymentMethod.type == 'bank').map((paymentMethod, index) => {
                                            return (
                                                <Pressable style={styles.infoLinkContainer} key={index} onPress={() => goPaymentMethodDetailScreen(paymentMethod)}>
                                                    <View style={styles.infoWrapper}>
