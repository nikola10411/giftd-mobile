...
import { addThankyou, addThankyouToContract, clearContactState, clearPaymentMethodState, clearThanksNoteState, getContactList, getFavoriteContactList, getFilteredContactList, getPaymentMethods, selectPaymentMethod, setContactInfo } from '../actions/giftAppAction';
...
interface ISelectPaymentMethodFromTransactionsScreenProps {
  navigation: NavigationProp<any>;
}

const SelectPaymentMethodFromTransactionsScreen: React.FC<ISelectPaymentMethodFromTransactionsScreenProps> = (props) => {
  ...
  const { paymentMethodList, transactionDetail, isAddThankyouSuccess1 } = useSelector((state: IRootState) => state.giftApp);
  ...
  const selectPaymentMethodHandle = (paymentMethod: PaymentMethod) => {
    ...
    if (transactionDetail && transactionDetail.sender_contact_id && paymentMethod) {
        const members = [transactionDetail.sender_contact_id];
        dispatch(addThankyouToContract(
            members,
            paymentMethod.stripe_token_id
        ));
    }
  };

  useEffect(() => {
      if (isAddThankyouSuccess1) {
          props.navigation.navigate('AddThankyouNote1Screen');
          dispatch(clearThanksNoteState());
      }
  }, [isAddThankyouSuccess1])

  // useEffect(() => {
  //     if (isAddThankyouSuccess) {
  //         dispatch(clearThanksNoteState());
  //         props.navigation.navigate('AddThankyouNoteScreen');
  //     }
  // }, [isAddThankyouSuccess])

  const sengotoPaymentMethodSettingdGift = () => {
      props.navigation.navigate('PaymentMethodSelectionFromHomeScreen');
  };
...
export default SelectPaymentMethodFromTransactionsScreen;
