...
import { addMember, deleteMember, getContactList, getFavoriteContactList, getFilteredContactList, getMembers, setMember, setMembersInfo } from '../actions/giftAppAction';
import { IRootState } from '../reducers';
import CheckBox from 'react-native-check-box'
import Icon2 from 'react-native-vector-icons/EvilIcons';

interface IEditInviteMembersScreenProps {
    navigation: NavigationProp<any>;
}

const EditInviteMembersScreen: React.FC<IEditInviteMembersScreenProps> = (props) => {
    ...
    const { contactList, favoriteList, eventDetailInfo, isSetMemberSuccess, eventMembers} = useSelector((state: IRootState) => state.giftApp)

    const addMembers = () => {
        console.log(toggleCheckBox);
        dispatch(setMember(eventDetailInfo.id, toggleCheckBox));
        if (isSetMemberSuccess) {
    ...
    useEffect(() => {
        if (eventMembers) {
            setToggleCheckBox(eventMembers.map((eventMember: any) => eventMember.contacts_id))
        }
    }, [eventMembers])
...
export default EditInviteMembersScreen;
